...
Requirements
Traverse the parking grid row by row (top to bottom).
Within each row, traverse left to right.
Identify the first occurrence of an available spot (0).
As soon as the first available spot is found:

   
Print its level (row index)
Print its spot (column index)
Stop the search immediately after finding the first available spot.
